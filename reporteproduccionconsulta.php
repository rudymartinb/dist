...
if( $post != null ){
	...
	if( count( $arr ) == 0 ){
		...
		*/
		$self = "reporteproduccionconsulta2.php";
	...
	$p->body->AddHTML( '<form name="reporte" action="'.$self.'" method="post">' );
		...
		$fecini = "01-".date("m-Y");
		$fecfin = "01-".date('m-Y',strtotime("+1 month"));
		$fecfin = FechaRestarDias( $fecfin, 1 );

	$p->body->AddHTML( '<input type=hidden value="1" id="nrotiradas" name="nrotiradas" >' );
	$txt = GenerarInput( "Desde Fecha", "fecini", "", 10, "fecini", $fecini, "Escriba la Fecha Inicial a buscar en formato DD-MM-AAAA" ); $p->body->AddHTML( $txt );
	$txt = GenerarInput( "Hasta Fecha", "fecfin", "", 10, "fecfin", $fecfin, "Escriba la Fecha Final a buscar en formato DD-MM-AAAA" ); $p->body->AddHTML( $txt );
