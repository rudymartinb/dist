...
$p->head->titulo = "Alta Tipo Documento";
...
if( $post != null ){
	$cod = $post['cod'];
	$des = $post['des'];
	if( sp_tipdoc_agregar( $db, $cod, $des ) ){
		$anexo = "Tipo de documento ".$cod." ".$des." agregado. ".$db->error;
	} else {
		$anexo = "Tipo de documento ".$cod." ".$des." ".$db->error;
...
$p->body->AddHTML( '<input class="adminput1" type=text value="" id="cod" name="cod" maxlength="2">' );
$p->body->AddHTML( '<div class="admmensa">Escriba los dos digitos del codigo a utilizar</div>' );

$p->body->AddHTML( '<div class="admitem">Descripcion:</div>' );
$p->body->AddHTML( '<input class="adminput2" type=text value="" id="des" name="des" maxlength="20">' );
$p->body->AddHTML( '<div class="admmensa">Escriba la descripcion</div>' );
...
function sp_tipdoc_agregar( $db, $codtd, $destd ){
